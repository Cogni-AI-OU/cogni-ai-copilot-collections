...
- Intro step with an embedded SVG architecture diagram (raw GitHub URL
  inside the description)
- Rich markdown per step with emoji section headers (`### [Video] Tour Player`)
...
- Almost entirely `directory` steps -- orients to every `src/` subdirectory
  without getting lost in files
- Conversational, beginner-friendly tone throughout
- `selection` on the opening step to highlight the exact entry in
  `package.json`
- Closes with a genuine thank-you and call-to-action

**Technique to copy:** Use directory steps as the skeleton of an onboarding tour -- they teach structure without
requiring the author to explain every file.
...
- `commands` array to run real VS Code commands mid-tour: the tour literally executes
  `codeQL.runQuery` when the reader arrives at that step
...
```json
{
  ...
  "description": "Click the **[Play] Run** button above. The results appear in the CodeQL Query Results panel."
}
```
...
**Technique to copy:** For quick/vibecoder tours, cut mercilessly. Four steps that drive action
beat twelve that explain everything.
...
- Uses **content-only checkpoint steps** (no `file` key) as progress milestones: "Check out your page! !"
  and "Try it out!" between coding tasks
...
**Technique to copy:** Checkpoint steps (content-only, milestone title) break up long tours and
give the reader a sense of progress.
...
- Three separate tours for three separate concerns (function code, IaC, CI/CD pipeline) -- each
  standalone but linked via `nextTour`
...
**Technique to copy:** For complex systems, write one tour per layer and chain them with `nextTour`.
Don't try to cover infrastructure + application code + CI/CD in one tour.
...
- Proves that "contributor onboarding" tours don't have to start with `main()` -- they start
  with whatever is confusing about this specific repo
